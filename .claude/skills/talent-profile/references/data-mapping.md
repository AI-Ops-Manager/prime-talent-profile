...
- 2段落目: いまの取り組み（projects の代表）と、評価されている点。数字があれば入れる
- 各段落3〜4行（全角120〜160文字）。3段落にしない

## 課題とおすすめポイント（`fit` / `points`。打診先が決まっているときだけ）

| 元 | 先 | 写し方 |
|---|---|---|
| 案件要件の MUST / WANT、商談メモの発言（`talent-pickup` レポートの「案件要件」） | `fit` | 読み手の悩みを一人称の1行に。3点。要件に無い悩みを足さない |
| `experiences` / `projects` の数字、レポートの「この案件で効くところ」 | `points` | 課題の順に対応させて4点。各1〜2行、実績の数字入り、太字は1箇所 |

2節を書くと1ページ目が 概要（1段落）→課題→おすすめポイント になり、対応領域は2ページ目の先頭に移る。

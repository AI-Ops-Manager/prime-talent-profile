...
- **用途**: 打診先や案件が分かっていれば「希望領域」の一行と、プロジェクトの並び順をそこに寄せる。
  さらに `fit`（こんな課題をお持ちの企業様に・3点）と `points`（おすすめポイント・4点）を書き、1ページ目を売り込みの構成にする。
...
- `fit` は読み手の悩みを相手の一人称で、各1行（全角30字以内）・3点。「広告費をかけているのに売上が伸びない。代理店任せで中身が見えない」のように、
  案件要件の MUST / WANT を悩みの言葉に言い換える。創作しない
- `points` は課題への答えになる実績を数字で、各1〜2行（全角50字目安）・4点・**太字**は1項目1箇所。
  順番は課題の順に対応させる。「唯一」「最も」のような相対表現は、同じ打診先に出す資料一式で整合するときだけ

...
- **用途**: 打診先や案件が分かっていれば「希望領域」の一行と、プロジェクトの並び順をそこに寄せる。
  さらに `fit`（解決できる課題・3点）と `points`（推薦理由・4点）を書く（2ページ目の先頭に入って経歴が続く。スキルは1ページ目、対応領域は3ページ目の先頭に移る。概要は1段落・実績は3件に）。
  `talent-pickup` を通っていれば、レポートの案件要件と「この案件で効くところ」がそのまま原稿になる。
  打診先が未定の汎用資料でも、本人の `projects` / `experiences` が実際に解いた課題を読み手企業の状況の文に
  言い換えれば書ける（要件の無い課題を足すのは創作）。依頼者が汎用の構成を望むなら2節とも書かない
- **写真**: 出す運用か伏せる運用か（`brand.json` の `defaults.hidePhoto` が正。個別に伏せたいときは `photo: null`）
- **単価**: 表示は `brand.json` の `defaults.rate` が既定（15,000円/h〜。注記なし）。AOM から個別の案内があるタレント（例: 20,000円/h〜）だけ `facts.rate` に書く。計算根拠を資料や `_note` に書かない
...
- 概要は2段落。1段落目=何者かと一番の強み（**太字**は1箇所）、2段落目=いまの取り組みと評価されている点。
  `fit` / `points` を書くときは**1段落**（3〜4行）にまとめる
- `fit` は読み手の状況を客観の文で、各1行（全角35字以内）・3点。「CRMを導入したが入力が定着せず、数値の信頼性が低い」のように、
  案件要件の MUST / WANT を課題の文に言い換える。口語・呼びかけ・感嘆にしない。創作しない
- `points` は課題に対応する実績を数字で、各1〜2行（全角50字目安）・4点・**太字**は1項目1箇所。
  順番は課題の順に対応させ、文末は「〜できる」「〜の知見を持つ」のような事実の言い切りにする。
  「唯一」「最も」のような相対表現は、同じ打診先に出す資料一式で整合するときだけ
- 経歴は新しい順に3〜4件（`fit` / `points` があるときは3件）。本文は「何を、どこからどこまで担ったか」を2文以内
- 実績（`projects`）は3〜5件。課題に合う2件を `featured: true` に（資料では「適合」のラベルが付く。汎用の資料では「注目」）。`description` が空のものは載せない
...
- `_note` に UUID と、確認が残っている点（稼働・所在地・単価）を書いておく（資料には出ない）。
  ただし `_note` にも実名・社名・連絡先は書かない（ファイルは残り続ける。伏せた理由は「現職のため」のように書く）
